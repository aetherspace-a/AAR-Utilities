...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('landingmusic')
        .setDescription('Plays landing music on a continuous loop.'),
    async execute(interaction) {
        ...
        const audioPath = path.join(__dirname, '../../audio/landing.mp3');
        if (!fs.existsSync(audioPath)) return interaction.reply({ content: '❌ `landing.mp3` missing.', flags: MessageFlags.Ephemeral });
        ...
        await interaction.editReply({ content: '🔁 **Now Looping:** Landing Music.' });
